...
void Computer::FindTheBestPlacementOfFigure(general* f, int NumPlayer, int Level1)
{
	...
	else
	{
		...
		BotMovesGrade* Pos = new BotMovesGrade[SIZE]{};
		...
		for (int i = 0; i < SIZE; i++)
		{
			...
			temp->update(GameBoard, temp->getSerial(), RIGHT, 1);
			...
			delete temp;
			temp = new Figure(f->getSerial(), NumPlayer, GameBoard);
		...
		while (f->getRotateNum() != besRotation&& f->update(GameBoard, f->getSerial(), CLOCKWISE, 1))
		{
			f->print(GameBoard);
		...
		while (LowColOfCurrPlacement(f) < Pos[besRotation].Getbestcol()&& f->update(GameBoard, f->getSerial(), RIGHT, 1))
		{
			f->print(GameBoard);
			Sleep(100);
		}
		while (LowColOfCurrPlacement(f) > Pos[besRotation].Getbestcol()&& f->update(GameBoard, f->getSerial(), LEFT, 1))
		{
			f->print(GameBoard);
			Sleep(100);
